...
blade_length = 200;      // Length of the blade section to be covered
blade_width_start = 48;  // Blade width at the sheath opening (handle end)
blade_thickness_edge = .6; // NEW: Blade thickness near the cutting edge [mm] (Defines INNER cavity)
blade_thickness_spine = 2.7; // NEW: Blade thickness near the spine [mm] (Defines INNER cavity)

blade_tip_offset = 23; // Y-coordinate of the tip (relative to center Y=0)
// Edge Control Points
edge_point_1_x = 10;
edge_point_2_x = 160;
edge_point_2_y = 15; // Negative for edge side from center

// Spine Control Points
spine_point_1_x = 1;
spine_point_2_x = 160;
spine_point_2_y = 34;  // Positive for spine side from center

wall_thickness = 1.5;      // Desired wall thickness of the sheath [mm]
clearance = 0.2;         // Space (air gap) between blade and inner wall [mm]

drainage_hole_diameter = 10;       // Diameter of drainage holes [mm]
